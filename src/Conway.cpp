...
void Conway::placeObject(std::string apgcode, int y, int x, bool opaque){
...
std::vector<std::vector<int> > Conway::generateObject(std::string apgcode){
	...
	for (int m = 0; m < num_states; m++){
		int num_z_2 = 0;
		int num_rows_2 = 1;
		for (int i = iter2; i < results.size(); i++){
			if (results[i] == "z"){
				num_z_2++;
				num_rows_2++;
				while(results[i+1] == "z"){
		...
		for (int j = 0; j < num_rows[m]; j++){
				...
				for (int k = 0; k < cont_rows[j].size(); k++){
					...
					if (cont_rows[j][k] == "w"){count+=2;}
						...
						temp_object[j*5][count] = m+1;
						count++;
					}
					else if (cont_rows[j][k] == "2"){
						temp_object[j*5+1][count] = m+1;
						count++;
					}
